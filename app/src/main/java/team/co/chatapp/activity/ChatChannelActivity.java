...
import team.co.chatapp.adapter.ThreadAdapter;
import team.co.chatapp.model.ChatChannel;
import team.co.chatapp.model.MessageThread;
import team.co.chatapp.model.NotificationPost;
import team.co.chatapp.model.User;
import team.co.chatapp.network.ApiClient;
import team.co.chatapp.network.ApiService;
...
//        }
    ...
    private void sendMessage() {
        if (intentChatId.isEmpty()) {
            ...
            sendMsg(new MessageThread(
                    AppSharedData.getUserData().getId().toString(),
                    HelperMethods.AESDecryptionMethod(mMessage.getText().toString()),
                    AppSharedData.getUserData().getName(),
                    "text", Calendar.getInstance().getTime().getTime())
            );
    ...
    private void createChannel(MessageThread messageThread) {
        ...
        users.add(0, AppSharedData.getUserData().getId());//sender
        users.add(1, user.getId().toString());//receiver

//        List<String> tokens = new ArrayList<String>();
//        tokens.add(0, AppSharedData.getDeviceToken());//sender
//        tokens.add(1, user.getToken());//receiver
...
//            userMap["lastMessageTime"] = messageThread.created
